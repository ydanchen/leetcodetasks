...
public class Solution {
    public int myAtoi(String str) {
        ...
        int sign = 1, i = 0, result = 0, max = Integer.MAX_VALUE / 10, current_digit = 0;
        if (i < number.length && (number[i] == '-' || number[i] == '+')) {
            sign = (number[i] == '-') ? -1 : 1;
            i++;
        }
        while (i < number.length && number[i] > 47 && number[i] < 58) {
            current_digit = number[i] - 48;
            if (result > max || (result == max && current_digit > 7)) {
